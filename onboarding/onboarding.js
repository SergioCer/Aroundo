...
resetDay.addEventListener("click", () => {highlightRemove(resetDayHighlight1);});
    ...
    startHighlight = highlight(startHandle);
    await wait(3000);
    ...
    highlightRemove(startHighlight);
    await wait(1000);
    endHighlight = highlight(endHandle);
    await wait(3000);
    const newEnd = Number(values[1]) - 12;
    slider.noUiSlider.set([newStart, newEnd]);
    highlightRemove(endHighlight);
    await wait(1000);
    startHighlight1 = highlight(startHandle);
    endHighlight1 = highlight(endHandle);
    await onboardingNext();
    highlightRemove(startHighlight1);
    highlightRemove(endHighlight1);
