...
async function loadAnalytics(){
  ...
  const {data,error}=await supabase
    .from("analytics")
    .select(`an_date, id_device, an_install, an_login, an_gps, an_open, an_share, an_more, an_info, an_marker, an_map, an_buy, an_book,
      devices (dv_device, dv_platform, dv_app, dv_entrance)`)
    .gte("an_date",from.toISOString().split("T")[0])
...
/* DATE HELPERS */
function cleanDate(d){return new Date(d.getFullYear(),d.getMonth(),d.getDate());}

/* RANGE CALENDARIO */
function getRange(type,date){let start;let end;
  date=cleanDate(date);
  if(type==="day"){start=new Date(date); end=new Date(date);}
  if(type==="week"){start=new Date(date);
...
function filter(data,type,date){
  ...
  return data.filter(x=>{const d=cleanDate(new Date(x.an_date));
...
function periodLabel(type,date){
  ...
  if(type==="week")
    return `${r.start.getDate()} ${r.start.toLocaleDateString("it-IT",{month:"long"})} - ${r.end.getDate()} ${r.end.toLocaleDateString("it-IT",{month:"long"})}`;
  if(type==="month")
...
function stats(data,allData){
  let devices=new Set();
  let s={open:0, login:0, install:0, share:0, marker:0, map:0, buy:0, book:0, more:0, info:0, gps:0, nogps:0, nonegps:0, app:0, web:0, 
         newUsers:0, returnUsers:0, retention:0, avgOpen:0, trust:0};
  let deviceState={};
  let gpsState={};
  let entranceState={};
  data.forEach(x=>{const id=x.id_device;
    const device=x.devices;
    if(id)
      devices.add(id);
    if(id && !(id in deviceState)){
      deviceState[id]={
        app:device?.dv_app||false,
        platform:device?.dv_platform||"Unknown"
      };
    }
    if(id)
      entranceState[id]=device?.dv_entrance||null;
    if(id && !(id in gpsState))
      gpsState[id]=x.an_gps;
    s.open+=x.an_open||0;
  ...
  Object.values(gpsState).forEach(gps=>{if(gps===true) s.gps++;
    else if(gps===false) s.nogps++;
    else s.nonegps++;
  });
  ...
  const previous=new Set(allData
  ...
  s.entranceTotal=Object.values(entrances)
    .reduce((a,b)=>a+b,0);
...
function platformStats(deviceState){
  ...
  return Object.entries(platforms)
    .map(([platform,count])=>({platform, count, percent:total ? Math.round((count/total)*100) : 0}));
}
...
function metric(icon,label,value,text,secondary=""){return`
    <div class="metric" title="${text}">
      ...
      <div class="metric-value">${value}${secondary?`<span class="metric-secondary"> (${secondary})</span>`:""}</div>
    </div>`;
}
...
function renderSection(type){
  ...
  document.getElementById(type+"Title").textContent=
    periodLabel(type,currentDate);
  document.getElementById(type+"Metrics").innerHTML=`
    <div class="metric-group">
      <h3>Use</h3>
      ${metric("📱","Devices",s.devices,"Unique devices")}
      ${metric("🌐","Web",s.web,"Web usage")}
      ${metric("📲","App",s.app,"App usage",
        s.devices?Math.round((s.app/s.devices)*100)+"%":"0%")}
    ...
    <div class="metric-group">
      ...
      ${metric("🔁","Return",s.returnUsers,
        "Returning devices",s.retention+"%")}
      ...
      ${metric("⚪","None",s.nonegps,"GPS Unknown")}
      ${metric("🎯","Trust",s.trust+"%","GPS trust")}
    </div>
    <div class="metric-group">
      <h3>Technology</h3>
      ${platforms.map(p=>
        metric("💻",p.platform,p.count,
          "Platform usage",p.percent+"%")
      ).join("")}
    </div>
  `;
}
...
/* CAMBIO PERIODO */
function move(type,value){if(type==="day") currentDate.setDate(currentDate.getDate()+value);
  ...
  if(cleanDate(currentDate)>today)
    currentDate=today;
  render();
}
...
document.querySelectorAll(".collapse-btn").forEach(btn=>{
  btn.onclick=function(){document.querySelectorAll(".period-section")
