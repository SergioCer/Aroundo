...
function extractAttributes(block, tag, attribute) {
  const re = new RegExp(`<${tag}\\b[^>]*\\b${attribute}\\s*=\\s*["']([^"']+)["'][^>]*>`, "gi");
  const result = [];
  for (const match of block.matchAll(re)) {
    if (match[1]) result.push(match[1]);} return result;
}
...
function flattenNodes(root) {
  const result = [];
  function walk(node) {result.push(node); for (const child of node.children) {walk(child);}}
  walk(root);
  return result;
}
...
function isHeaderFooter(node) {
  return (node.tag === "header" || node.tag === "footer");
}
...
function candidateContainers(root) {
  ...
  return nodes
    .filter(node => {
      if (node.tag === "#root" || isHeaderFooter(node)) {return false;}
      if (node.end <= node.start) {return false;}
      const length = node.end - node.start;
      /* Evitiamo contenitori enormi. Il root/body non deve diventare automaticamente un evento. */
      if (length > 250000) {return false;}
      return ["article", "section", "div", "li", "main", "aside"].includes(node.tag);
    })
    /* dal più piccolo al più grande: vogliamo trovare il contenitore minimo che contiene la firma. */
 ...
 function evaluateContainer(node, html, url, categoryDictionary) {
   ...
   /* Escludiamo solo strutture palesemente tecniche.
     Non facciamo blacklist di "social", "correlati", "pubblicità", ecc. */
   if (/^<(?:script|style|noscript|svg)\b/i.test(block.trim())) {return null;}
   const previousUrl = CURRENT_URL;
   CURRENT_URL = url;
   const signals = analyzeSignals(block, categoryDictionary);
   CURRENT_URL = previousUrl;
   if (!signals) return null;
   if (signals.classification === "non-evento") {return null;}
   return {node, block, signals};
 }
...
function findSemanticCandidates(html, url, categoryDictionary) {
  ...
  for (const node of nodes) {
    const evaluated = evaluateContainer(node, html, url, categoryDictionary);
    if (!evaluated) continue;
    candidates.push(evaluated);
  }
  /* Ora dobbiamo eliminare i contenitori che sono semplicemente genitori di un candidato già trovato.
    Il candidato più piccolo viene considerato quello semanticamente più preciso. */
  ...
  for (const candidate of candidates) {
    ...
    /* Se il candidato è completamente contenuto dentro un candidato già accettato, non lo analizziamo come nuovo evento. */
    const insideAccepted = accepted.some(existing => start >= existing.node.start && end <= existing.node.end);
    if (insideAccepted) {continue;}
    /* Se invece questo candidato contiene un candidato precedente, il candidato precedente è quello semanticamente più preciso. */
    const containsAccepted = accepted.some(existing => start <= existing.node.start && end >= existing.node.end);
    if (containsAccepted) {continue;}
    accepted.push(candidate);
    consumed.push({start, end});
  }
  return {candidates: accepted, consumed};
}
...
http.createServer(
  async (req, res) => {
    ...
    if (!url) {return res.end(JSON.stringify({error: "URL mancante"}, null, 2));}
    try {
      const page = await extractPage(url);
      const eventi = findEvents(page.content, url, categoryDictionary);
      return res.end(JSON.stringify({acquisizione: page.acquisizione, eventi,
            /* lasciamo l'HTML disponibile al viewer per il debug. */
            content: page.content}, null, 2));
    } catch (error) {return res.end(JSON.stringify({error: error.message}, null, 2));}
  }
).listen(3003, () => console.log("HTML Semantic Extractor attivo sulla porta 3003"));
}
startServer();
