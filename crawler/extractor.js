...
http.createServer(async(req,res)=>{const requestUrl=new URL(req.url,"http://localhost:3000"),url=requestUrl.searchParams.get("url");
...
if(requestUrl.pathname!=="/"){res.writeHead(404,{"Content-Type":"application/json; charset=utf-8"});return res.end(JSON.stringify({error:"Risorsa non trovata"}));}
